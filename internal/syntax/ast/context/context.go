...
func (rc *ResolveContext) Bind(fields ...fields.Field) {
	current := rc.CurrentScope()
	current.fields = append(current.fields, fields...)
}

func (rc *ResolveContext) Lookup(relationName, name string) (fields.Field, error) {
	for i := len(rc.Scopes) - 1; i >= 0; i-- {
		...
		if len(candidates) > 1 {
			return fields.Field{}, fmt.Errorf("ambiguous field %q", name)
		}
	...
	return fields.Field{}, fmt.Errorf("unknown field %q", name)
}
